...
export class AuthFormComponent implements OnInit {
  ...
  async onSubmit(event: Event) {
      ...
      try {
        ...
        try {
          const existingUser = await firstValueFrom(
            this.userProfilesService.getByEmail(decodeURIComponent(email))
          );
          if (existingUser) {
    ...
    if (this.mode === 'login') {
      try {
        ...
        const response = await this.authService.signInUser(email, password);
        ...
        if (response.status === 200) {
          ...
          this.errorMessage =
            'Inicio de Sesion Fallido. Por favor revise sus datos e intente nuevamente.';
        }
      } catch (err: any) {
        console.error('Error al iniciar sesión:', err);
        this.errorMessage = err?.response?.data?.message || 'Error durante el inicio de sesión.';
      }
    }
    ...
    if (this.mode === 'reset') {
      ...
      if (errors.length > 0) {
        this.errorMessage =
          'La nueva contraseña no cumple con:\n- ' + errors.join('\n- ');
        return;
      }
      ...
      try {
        ...
        // Si lanza error, la contraseña es distinta, lo cual está bien
      }
      ...
      try {
        ...
        if (changeResponse.status === 200) {
          ...
          this.errorMessage =
            changeResponse.data?.message || 'Error al restablecer la contraseña.';
        }
      } catch (err: any) {
  ...
  private async storeSessionData(data: any, email: string) {
    ...
    try {
      const existingUser = await firstValueFrom(
        this.userProfilesService.getByEmail(email)
      );
